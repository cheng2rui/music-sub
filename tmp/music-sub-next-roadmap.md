# Music Sub 后续重开发路线（产品化与标签治理优化）

> 目标版本基线：Music Sub v0.7.12+  
> 参考资料：`/tmp/reference-frontend`、`tmp/mobile-ui-audit-report.md`、`tmp/mtw-scrape-audit-report.md`  
> 取舍原则：优先补「产品感 + 稳定性 + 可观测性」，不追求照搬 成熟产品的 Vuetify/插件体系，也不把 mtw 的全量标签编辑一次性搬完。

## 0. 当前判断

Music Sub 现在已经吃掉了一批最关键的低垂果实：

- 产品化方向：底部导航已收口为「发现 / 搜索 / 曲库 / 任务 / 更多」，设置页已有分组 tabs，移动端播放器和卡片化已明显改善。
...
成熟产品的 `dashboard.vue` 值得抄的不是拖拽本身，而是：
...
成熟产品的插件市场很成熟，但 Music Sub 目前体量不适合立刻做插件系统。更适合借鉴两个形态：
...
成熟产品的任务和日志有强烈的运维产品感：状态、进度、动作、日志能互相跳转。
...
成熟产品的通知中心值得抄“统一状态入口”的思想。Music Sub 可分两步：
...
成熟产品的 `useDynamicButton` + footer floating action 很适合移动端。Music Sub 可做极简版：
...
成熟产品的移动端强在浮层体系：bottom sheet、shared dialog、footer dock、安全区统一。
